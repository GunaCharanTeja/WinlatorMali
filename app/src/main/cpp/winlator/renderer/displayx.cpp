...
void DisplayX::createWindowControl(Window *window) {
    if (!window->parent || !window->inputOutput) {
        printf("createWindowControl: skipping window %d (hasParent: %d, inputOutput: %d)", window->id, window->parent != nullptr, window->inputOutput);
        return;
    }
...
void DisplayX::destroyWindowControl(Window *window) {
    ...
    printf("destroyWindowControl: releasing control %p for window %d", window->control, window->id);
...
void DisplayX::mapWindow(Window *window) {
    if (!window->control) {
        printf("mapWindow: window %d has no control!", window->id);
        return;
    }
    
    std::string unviewable = windowManager->getUnviewableWMClass();
    if (!unviewable.empty() && !window->className.empty() && window->className.find(unviewable) != std::string::npos) {
        window->enabled = false;
    } else {
        window->enabled = true;
    }
        
    printf("mapWindow: showing window %d (control %p, enabled %d, class '%s', unviewable '%s')",
           window->id, window->control, window->enabled, window->className.c_str(), unviewable.c_str());
...
void DisplayX::updateCursorPosition() {
    ...
    jint id = pointWindowObj ? env->GetIntField(pointWindowObj, cache->windowID) : -1;
    if (pointWindowObj) env->DeleteLocalRef(pointWindowObj);
    ...
    if (cursorVisible || (cursor && cursor->visible)) {
        if (pfnASurfaceTransactionSetPosition) {
            ...
            auto& cursorImage = (cursor != nullptr && cursor->image) ? cursor->image : rootCursor->image;
            ARect src{};
            ARect dst = {
                .left = x,
                .top = y,
...
void DisplayX::createRootCursorControl() {
    ...
    if (!rootCursor || !rootWindow || !rootWindow->control) return;
...
void DisplayX::showCursor() {
    ...
    auto rootCursor = cursorManager->getRootCursor();
    if (!rootCursor || !cursorManager->control) return;
