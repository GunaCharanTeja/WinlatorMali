...
public class ControlElement {
    ...
    public void draw(Canvas canvas) {
        ...
        float opacity = inputControlsView.getOverlayOpacity();

        boolean isPressed = false;
        for (boolean state : states) if (state) { isPressed = true; break; }

        // Refined Minimal Premium Transparent Style
        // Darker interiors for better readability on bright scenes
        int fillColor = ColorUtils.setAlphaComponent(Color.BLACK, (int) (opacity * 255 * (isPressed ? 0.65f : 0.45f)));
        // Higher contrast borders while keeping them thin and soft
        int borderColor = Color.argb(255, 230, 230, 230); // Soft grey-white
        int strokeColor = ColorUtils.setAlphaComponent(selected ? inputControlsView.getSecondaryColor() : borderColor, (int) (opacity * 255 * (isPressed ? 1.0f : 0.8f)));
        int textColor = ColorUtils.setAlphaComponent(Color.WHITE, (int) (opacity * 255 * 0.95f));
        int glowColor = ColorUtils.setAlphaComponent(Color.WHITE, (int) (opacity * 255 * (isPressed ? 0.20f : 0.10f)));
        ...
        if (isPressed) {
            paint.setShadowLayer(snappingSize * 0.3f * scale, 0, 0, glowColor);
        } else {
            paint.setShadowLayer(snappingSize * 0.2f * scale, 0, 0, glowColor);
        }
        ...
        switch (type) {
            case BUTTON: {
                ...
                // Identify shoulder buttons for capsule style
                String displayText = getDisplayText();
                boolean isShoulderButton = displayText.matches("(?i)L[12]|R[12]|LT|RT|LB|RB");
                ...
                if (isShoulderButton) {
                    ...
                    drawShape(canvas, boundingBox, snappingSize, paint);
                    paint.setStyle(Paint.Style.STROKE);
                    paint.setColor(strokeColor);
                    drawShape(canvas, boundingBox, snappingSize, paint);
                }
                ...
                if (iconId > 0) {
                    ...
                    canvas.drawText(displayText, x, (y - ((paint.descent() + paint.ascent()) * 0.5f)), paint);
                }
                break;
            }
            case D_PAD: {
            ...
            case STICK: {
                ...
                // Outer Ring - Improved visibility for bright scenes
                paint.setShadowLayer(0, 0, 0, 0);
                paint.setStyle(Paint.Style.FILL);
                paint.setColor(ColorUtils.setAlphaComponent(Color.BLACK, (int) (opacity * 255 * 0.35f)));
                canvas.drawCircle(cx, cy, outerRadius, paint);

                paint.setStyle(Paint.Style.STROKE);
                paint.setColor(ColorUtils.setAlphaComponent(borderColor, (int) (opacity * 255 * 0.75f)));
                canvas.drawCircle(cx, cy, outerRadius, paint);
                ...
                // Remove joystick glow completely as requested
                paint.clearShadowLayer();

                // Thumbstick Body - Smooth dark translucent
                paint.setStyle(Paint.Style.FILL);
                paint.setColor(fillColor);
                canvas.drawCircle(thumbstickX, thumbstickY, thumbRadius, paint);

                // Thumbstick Outline - Thin soft contrast border
                paint.setStyle(Paint.Style.STROKE);
                paint.setColor(strokeColor);
                canvas.drawCircle(thumbstickX, thumbstickY, thumbRadius, paint);
                break;
            ...
            case TRACKPAD: {
                ...
                float offset = strokeWidth * 3.0f;
                float innerStrokeWidth = strokeWidth * 1.2f;
    ...
    private void drawShape(Canvas canvas, Rect boundingBox, int snappingSize, Paint paint) {
        switch (shape) {
            case CIRCLE:
