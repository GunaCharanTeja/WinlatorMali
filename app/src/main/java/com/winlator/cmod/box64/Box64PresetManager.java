...
public abstract class Box64PresetManager {
    ...
    public static int getNextPresetId(Context context, String prefix) {
        ...
        for (String[] preset : customPresetsIterator(prefix, context)) {
            try {
                String id = preset[0].toUpperCase(Locale.ENGLISH);
                if (id.startsWith(Box64Preset.CUSTOM + "-")) {
                    maxId = Math.max(maxId, Integer.parseInt(id.replace(Box64Preset.CUSTOM + "-", "")));
                }
            } catch (Exception e) {}
        }
        return maxId+1;
    ...
    public static void editPreset(String prefix, Context context, String id, String name, EnvVars envVars) {
        ...
        String sanitizedEnvVars = envVars.toString().replace("|", "");
        ...
        if (id != null) {
            ...
            String preset = Box64Preset.CUSTOM+"-"+getNextPresetId(context, prefix)+"|"+sanitizedName+"|"+sanitizedEnvVars;
