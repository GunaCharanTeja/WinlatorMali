    ...
    private class FXAAMaterial extends ScreenMaterial {
        ...
        protected String getFragmentShader() {
            ...
            return String.join("\n", new CharSequence[]{
                    ...
                    "    vec3 rgbNW = texture2D(screenTexture, vUV + vec2(-1.0, -1.0) * invResolution).rgb;",
                    "    vec3 rgbNE = texture2D(screenTexture, vUV + vec2( 1.0, -1.0) * invResolution).rgb;",
                    "    vec3 rgbSW = texture2D(screenTexture, vUV + vec2(-1.0,  1.0) * invResolution).rgb;",
                    "    vec3 rgbSE = texture2D(screenTexture, vUV + vec2( 1.0,  1.0) * invResolution).rgb;",
                    ...
                    "    vec4 rgbA = 0.5 * (",
                    "        texture2D(screenTexture, vUV + dir * (1.0 / 3.0 - 0.5)) +",
                    "        texture2D(screenTexture, vUV + dir * (2.0 / 3.0 - 0.5)));",
                    "    vec4 rgbB = rgbA * 0.5 + 0.25 * (",
                    "        texture2D(screenTexture, vUV + dir * -0.5) +",
                    "        texture2D(screenTexture, vUV + dir *  0.5));",
